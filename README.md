...
As an owner of a Sony TV I downloaded this application to control it from my PC.  There are just a few changes I had to make so my TV would work.

Issues I've had so far:

My TV will not register with the application using the Forms section.  It will register with the console application but nothing is saved to a file so I could load into the forms application.

Another issue is that upon clicking the Discover All Button on the forms application, the application will exit with an unhandled exception error.

I will attempt to fix these issues with any changes I can make.
